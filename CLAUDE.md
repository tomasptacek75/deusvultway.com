...
- **Wordmark font changed to blackletter (2026-08-06)** — the "DEUS VULT WAY" wordmark in `AppShell.jsx`, `Landing.jsx`, and `Login.jsx` headers now renders in a blackletter/Old-English-style font via a new `font-gothic` Tailwind class, instead of the shared `font-display` (Oswald) used everywhere else. The user asked for a specific paid font ("CL Antique No.31" from centurylibrary.com/Creative Market) — that couldn't be sourced or embedded (no license), so this uses **Old London** instead, a freeware Old English/Textura font by Typographer Mediengestaltung (Dieter Steffmann), under 1001Fonts' "Free For Commercial Use" license. Self-hosted at `frontend/public/fonts/old-london.ttf` (not on Google Fonts) via `@font-face` in `index.css`; `tailwind.config.js`'s `gothic` family points at it. **First attempt used Google Fonts' UnifrakturMaguntia** (German Fraktur) as a quick free substitute — the user correctly rejected it after a letter-by-letter comparison: Fraktur has a structurally different lowercase letter skeleton (rounder bowls) than the English Old-English-Text/Textura style the reference actually was, despite both being loosely "blackletter." Old London and Canterbury (same Steffmann/1001Fonts family) were downloaded and rendered side-by-side for comparison before picking Old London for its rounder, more matching capital-letter swashes. If this ever needs revisiting (e.g. David gets a license for the actual CL Antique No.31), swap the `@font-face` src and the `gothic` family in `tailwind.config.js` — don't touch `font-display`, which every numeric/stat display across the app still depends on and must stay Oswald.
